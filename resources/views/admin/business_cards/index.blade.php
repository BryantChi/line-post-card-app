    ...
    <div class="content px-3">
        ...
        @if(Auth::user()->isSubUser())
            <div class="alert alert-warning">
                <strong>注意：</strong>每位會員只能擁有一組AI數位名片，請聯繫主帳號管理員進行調整。
            </div>
        @endif
        {{-- 超級管理員和主帳號可以擁有多組AI數位名片及子帳號 --}}
        @if(Auth::user()->isMainUser() || Auth::user()->isSuperAdmin())
            <div class="alert alert-info">
                <strong>提示：</strong>管理員帳號可以擁有多組AI數位名片，請根據需要進行管理。
            </div>
        @endif
...
@push('page_scripts')
    ...
    <script>
        function startBusinessCardsTour() {
            ...
            const stepData = [
                { selector: '[data-step="1"]', defaultIntro: "<strong>新增AI數位名片：</strong><br>點擊這裡開始建立您的第一張AI數位名片。您可以設定名片的標題、描述等基本資訊。" },
                @if(Auth::user()->isMainUser() || Auth::user()->isSuperAdmin())
                { selector: '[data-step="2"]', defaultIntro: "<strong>查看所有名片：</strong><br>如果您是管理員，可以點擊這裡查看系統中所有使用者建立的AI數位名片。", optional: true },
                @endif
                { selector: '[data-step="3"]', defaultIntro: "<strong>名片列表：</strong><br>這裡是您所有AI數位名片的列表。您可以從這裡管理每一張名片的卡片內容、編輯名片資訊、預覽名片效果或刪除不再需要的名片。" },
                { selector: '#businessCards-table tbody tr:first-child [data-step="4"]', defaultIntro: "<strong>查看名片：</strong><br>點擊此按鈕查看AI數位名片的詳細資訊及預覽。", optional: true },
                { selector: '#businessCards-table tbody tr:first-child [data-step="5"]', defaultIntro: "<strong>編輯名片：</strong><br>點擊此按鈕編輯AI數位名片的基本設定，例如標題、描述等。", optional: true },
                { selector: '#businessCards-table tbody tr:first-child [data-step="6"]', defaultIntro: "<strong>管理卡片：</strong><br>點擊此按鈕管理此AI數位名片包含的「卡片」。您可以在此新增、編輯、排序或刪除卡片。", optional: true },
                { selector: '#businessCards-table tbody tr:first-child [data-step="7"]', defaultIntro: "<strong>刪除名片：</strong><br>點擊此按鈕刪除此AI數位名片。請注意，此操作無法復原。", optional: true }
            ];
