...
@section('content')
    ...
    <div class="content px-3">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card" data-step="5" data-intro="這裡是數位名片的基本資訊，包括標題、狀態、分享連結等。">
            ...
            <div class="col-md-6">
                <div class="card" data-step="7" data-intro="這裡列出了此數位名片目前包含的所有「卡片」。您可以快速查看它們的標題和狀態。">
                    <div class="card-header">
                        <h3 class="card-title">數位名片-卡片列表</h3>
                        <div class="card-tools">
                            <a href="{{ route('admin.businessCards.bubbles.index', $businessCard->id) }}" class="btn btn-sm btn-primary"
                               data-step="8" data-intro="同樣可以點擊這裡進入「管理數位名片-卡片」頁面，進行新增、編輯或排序。">
                                <i class="fas fa-th-large"></i> 管理數位名片-卡片
                            </a>
                        </div>
                    </div>
                    <div class="card-body p-0">
                        @if($bubbles->isEmpty())
                            <div class="alert alert-warning m-3">
                                尚未添加任何氣泡卡片。點擊上方「管理數位名片-卡片」按鈕開始創建。
                            </div>
                        @else
                ...
                <div class="card mt-4" data-step="9" data-intro="這裡是此數位名片最終生成的 LINE Flex Message JSON 結構。您可以複製此結構到 LINE Flex Message Simulator 中進行更精確的預覽。">
                    <div class="card-header">
                    ...
                    <div class="card-body">
                        ...
                        @else
                            <div class="alert alert-warning">
                                尚未生成 Flex JSON。請先添加數位名片-卡片，然後點擊「重新生成 Flex JSON」按鈕。
                            </div>
                        @endif
                    </div>
                </div>
...
<script>
    ...
    function startShowCardTour() {
        ...
        const stepData = [
            { selector: '[data-step="1"]', defaultIntro: "<strong>返回：</strong><br>點擊這裡返回數位名片列表頁面。" },
            { selector: '[data-step="2"]', defaultIntro: "<strong>編輯：</strong><br>點擊這裡編輯此數位名片的基本資訊，如名稱、描述等。" },
            { selector: '[data-step="3"]', defaultIntro: "<strong>管理卡片：</strong><br>點擊這裡管理此數位名片包含的所有「卡片」內容。您可以在該頁面新增、編輯、排序或刪除卡片。" },
            { selector: '[data-step="4"]', defaultIntro: "<strong>預覽：</strong><br>點擊這裡在新分頁中預覽此數位名片的實際分享效果。" },
            { selector: '[data-step="5"]', defaultIntro: "<strong>基本資訊區塊：</strong><br>這裡是數位名片的基本資訊，包括標題、狀態、分享連結等。" },
            { selector: '[data-step="6"]', defaultIntro: "<strong>分享連結：</strong><br>這是此數位名片的公開分享網址。您可以複製此連結並分享給他人。點擊右側的複製按鈕即可複製。" },
            { selector: '[data-step="7"]', defaultIntro: "<strong>卡片列表區塊：</strong><br>這裡列出了此數位名片目前包含的所有「卡片」。您可以快速查看它們的標題和狀態。" },
            { selector: '[data-step="8"]', defaultIntro: "<strong>管理卡片按鈕：</strong><br>同樣可以點擊這裡進入「管理數位名片-卡片」頁面，進行新增、編輯或排序。" },
            { selector: '[data-step="9"]', defaultIntro: "<strong>LINE Flex JSON 預覽：</strong><br>這裡是此數位名片最終生成的 LINE Flex Message JSON 結構。您可以複製此結構到 LINE Flex Message Simulator 中進行更精確的預覽。" },
