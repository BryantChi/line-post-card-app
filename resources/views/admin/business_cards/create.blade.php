...
@section('content')
    ...
    <div class="content px-3">
        ...
        <div class="card">
            ...
            <div class="card-body" data-step="fields" data-intro="請填寫以下AI數位名片的基本資訊。">
                <div class="row">
            ...
            <div class="card-footer">
                {!! Form::submit('儲存', ['class' => 'btn btn-primary', 'data-step="save_create" data-intro="填寫完畢後，點擊這裡儲存新的AI數位名片。"']) !!}
                <a href="{{ route('admin.businessCards.index') }}" class="btn btn-default" data-step="cancel_create" data-intro="點擊這裡取消建立，並返回列表頁面。">取消</a>
                <div class="float-right text-muted" data-step="info_create" data-intro="建立名片後，您就可以開始為這張名片新增和管理多個「卡片」內容了。">建立後，您可以繼續添加AI數位名片-卡片</div>
            </div>
...
@push('page_scripts')
    ...
    <script @cspNonce>
        function startCreateCardTour() {
            ...
            const stepData = [
                { selector: '[data-step="1"]', defaultIntro: "<strong>卡片名稱：</strong><br>請為您的AI數位名片設定一個明確的名稱，方便您辨識。" },
                { selector: '[data-step="2"]', defaultIntro: "<strong>副標題：</strong><br>可選填。如果您的名片需要一個副標題，請在此輸入。" },
                { selector: '[data-step="3"]', defaultIntro: "<strong>頭像/Logo：</strong><br>上傳代表此AI數位名片的頭像或Logo圖片。" },
                { selector: '[data-step="4"]', defaultIntro: "<strong>啟用狀態：</strong><br>勾選此項以啟用此AI數位名片，使其可被分享和查看。" },
                { selector: '[data-step="5"]', defaultIntro: "<strong>簡介內容：</strong><br>在此輸入關於此AI數位名片的簡短介紹或說明。" },
                { selector: '[data-step="save_create"]', defaultIntro: "<strong>儲存：</strong><br>填寫完畢後，點擊這裡儲存新的AI數位名片。" },
