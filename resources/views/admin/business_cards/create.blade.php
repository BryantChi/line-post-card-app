...
@push('page_scripts')
    ...
    <script>
        function startCreateCardTour() {
            ...
            const stepData = [
                { selector: '[data-step="1"]', defaultIntro: "<strong>卡片名稱：</strong><br>請為您的電子名片設定一個明確的名稱，方便您辨識。" },
                { selector: '[data-step="2"]', defaultIntro: "<strong>副標題：</strong><br>可選填。如果您的名片需要一個副標題，請在此輸入。" },
                { selector: '[data-step="3"]', defaultIntro: "<strong>頭像/Logo：</strong><br>上傳代表此電子名片的頭像或Logo圖片。" },
                { selector: '[data-step="4"]', defaultIntro: "<strong>啟用狀態：</strong><br>勾選此項以啟用此電子名片，使其可被分享和查看。" },
                { selector: '[data-step="5"]', defaultIntro: "<strong>簡介內容：</strong><br>在此輸入關於此電子名片的簡短介紹或說明。" },
                { selector: '[data-step="save_create"]', defaultIntro: "<strong>儲存：</strong><br>填寫完畢後，點擊這裡儲存新的電子名片。" },
